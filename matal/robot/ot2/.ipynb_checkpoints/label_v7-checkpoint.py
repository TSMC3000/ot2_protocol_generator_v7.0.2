...
class LabelMaker:
        ...
        for i in range(self.c.N_COLS):
            for j in range(self.c.N_ROWS):
                ...
                p_fancy = FancyBboxPatch((bb.xmin, bb.ymin), bb.width, bb.height,
                                         ...
                                         ec=(0,0,0,0.1), fc='white', linestyle='-', lw=border_width)
    ...
    def draw_dish_comp_half_label(self, ax: matplotlib.axes.Axes, **kwargs):
        k = load_kwargs(kwargs, 'blue')
        self._draw_owner(ax, k, y_loc=0.7)
        
        fontsize = 5.5

        ax.text(0.3 * self.c.CELL_W, 0.9 * self.c.CELL_H, k.sid, color=k.color, font=FONT_MONOSPACE_BOLD, fontsize=6,
                horizontalalignment='center', verticalalignment='top')
        ax.text(0.175 * self.c.CELL_W, 0.4 * self.c.CELL_H, '\n'.join(k.additional_flags.split(', ')), color='black', font=FONT_MONOSPACE, fontsize=fontsize,
                horizontalalignment='left', verticalalignment='center')
        
        ax.text(0.1 * self.c.CELL_W, 0.31 * self.c.CELL_H, f's-{k.slot}', color='grey', font=FONT_MONOSPACE, fontsize=6,
                horizontalalignment='center', verticalalignment='bottom')
        ax.text(0.1 * self.c.CELL_W, 0.31 * self.c.CELL_H, f'w-{k.well}', color='grey', font=FONT_MONOSPACE, fontsize=6,
                horizontalalignment='center', verticalalignment='top')
...
#         ax.text(0.1 * self.c.CELL_W, 0.35 * self.c.CELL_H, f's-{k.slot}', color='grey', font=FONT_MONOSPACE, fontsize=6,
#                 horizontalalignment='center', verticalalignment='bottom')
#         ax.text(0.1 * self.c.CELL_W, 0.32 * self.c.CELL_H, f'w-{k.well}', color='grey', font=FONT_MONOSPACE, fontsize=6,
#                 horizontalalignment='center', verticalalignment='top')
...
def _new_label_page(**kwargs):
    fig = plt.figure(figsize=(8.5, 11))
    lm = LabelMaker(**kwargs)
    axes = lm.create_axes(fig)

    return lm, fig, axes


def base64_hash(o: object, digest_size: int = 15) -> str:
    return base64.b32encode(hashlib.blake2b(
        pickle.dumps(o), digest_size=digest_size).digest()).decode('utf-8')


def new_label_page(**kwargs):
    cache_tag = base64_hash(kwargs)
    LABEL_CACHE_DIR.mkdir(parents=True, exist_ok=True)
    try:
        with open(LABEL_CACHE_DIR / f'label_fig.{cache_tag}.pkl', 'rb') as f:
            lm, fig, axes = pickle.load(f)
    except Exception as e:
        lm, fig, axes = _new_label_page(**kwargs)
        with open(LABEL_CACHE_DIR / f'label_fig.{cache_tag}.pkl', 'wb') as f:
            pickle.dump((lm, fig, axes), f)
