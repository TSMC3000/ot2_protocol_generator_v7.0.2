...
class OT2Designer:
    ...
    def transfer_to_dst(self, src_sid, dst_slot, dst_well, vol):
        ...
        while vol > 0:
            ...
            vol_valid = min(vol_available, self.ot2_proc.pipette_max_vol)
            vol_i = vol if vol <= vol_valid else vol_valid
            if 0 < vol - vol_i < self.ot2_proc.pipette_min_vol:
                vol_i -= self.ot2_proc.pipette_min_vol
            ...
            self.transfers.append(AETransferStep(src_sid, slot, well, dst_sid, dst_slot, dst_well, vol_i,
                                                 self.wells[dst_slot][dst_well].vol))
            if vol_i < self.ot2_proc.pipette_min_vol:
                # Should never happen
    ...
    def generate_tip_steps(self, src_order=None):
        ...
        for transfer in transfers:
            ...
            if tip_sid != src_sid:
                ...
                if has_tip:
                    self.steps.append(AEDropTipStep(12, 'A1'))
                self.steps.append(AEPickUpTipStep(slot, well))
                num_tips_used += 1
                tip_sid = src_sid
                has_tip = True
            ...
            for well in self.wells[slot].keys():
                ...
                if sid and sid[0].islower() and vol > 0:
                    ...
                    mixing_transfers.extend([
                        AETransferStep(sid, slot, well, sid, slot, well, mix_vol, vol) for _ in range(repeats)])
...
def get_default_configs():
    c = dict(
        ...
        pipette_max_vol=800,
        pipette_min_vol=100,
        pipette_alt_max_vol=20,
        pipette_alt_min_vol=1,
        air_gap_vol=100,
...
class OT2ProcGenerator:
    ...
    def _old_loader(self, input_name, config_fmt: str = 'yaml', in_dir: str = None):
        raise NotImplementedError()
        # self.input_name = input_name
        # in_dir = INPUT_DIR if in_dir is None else Path(in_dir)
        #
        # if config_fmt == 'json':
        #     config_name = input_name + '.configs.json'
        #     with open(in_dir / config_name, 'r') as f:
        #         self.configs.update(json.load(f))
        # elif config_fmt == 'yaml':
        #     config_name = input_name + '.configs.yml'
        #     with open(in_dir / config_name, 'r') as f:
        #         self.configs.update(yaml.safe_load(f))
        # else:
        #     raise ValueError(f'Unknown config file format: {config_fmt}.')
        #
        # self.configs = Bunch(self.configs)
        #
        # if not self.configs.sample_file:
        #     self.configs.sample_file = f'{input_name}.samples.csv'
        # sample_path = in_dir / self.configs.sample_file
        #
        # self.samples = pd.read_csv(sample_path, dtype={'additional_flags': str}, keep_default_na=False)
    ...
    def __init__(self, sample_df: Union[str, pd.DataFrame], config_fmt: str = 'yaml',
        ...
        if isinstance(sample_df, str):
            warnings.warn('Init OT2ProcGenerator() with input_name is deprecated. '
                          ...
                          DeprecationWarning)
            self._old_loader(sample_df, config_fmt=config_fmt, in_dir=in_dir)
        ...
        if (self.configs.proj_code is None) and (isinstance(self.configs.batch_sid, str)) and (len(self.configs.batch_sid) > 2):
    ...
    def _check_configs(self):
        ...
        assert self.configs.label_grouping in ['by_type',
            ...
            for _ in range(n_replicates + 1):
                for i, sample in enumerate(samples):
                    ...
                    for src_i in range(1, self.configs.n_sources_per_sample + 1):
                        ...
                        line.append(src_sid)
                        line.append(getattr(sample, f'src{src_i}_frc') / self.configs.frc_one)
                        line.append(src_vol)
                    ...
                    if self.configs.water_target_vol is None:
                        if self.configs.water_target_mass_loading is not None:
                            water_target_vol = round(sample.mass * self.configs.scaling_factor / self.configs.water_target_mass_loading)
    ...
    def _gen_ot2_json(self, proc_sid: str, designer: OT2Designer, desc: str = None):
        with open(ASSETS_DIR / 'templates.json') as f:
        ...
        ingredients = {i: {
            'name': sid, 'description': None, 'serialize': False, 'liquidGroupId': i
        } for i, sid in enumerate(all_src_sid)}
        ...
        src_id_lut = {sid: i for i, sid in enumerate(all_src_sid)}
        pipette_id = '071e47a0-a461-11eb-a66e-01240ee358f3'
        ...
        step_pd = pd.DataFrame(
            columns=['idx', 'step_num', 'type', 'slot', 'well', 'sid', 'vol', 'dst_tot_vol', 'rate', 'offset', 'delay'])
        ...
        for i, step in enumerate(designer.steps):
            if isinstance(step, AEDropTipStep):
                command = [{
                    ...
                    'params': {
                        'pipette': pipette_id,
                        'labware': designer.labware_map[12].uuid_path,
                        'well': 'A1',
                    }
                }, ]
                step_pd.loc[len(step_pd)] = [len(step_pd), i, 'dropTip', 12, 'A1', '', na, na, na, na, na]
            elif isinstance(step, AEPickUpTipStep):
                command = [{
                    'command': 'pickUpTip',
                    'params': {
                        'pipette': pipette_id,
                        'labware': designer.labware_map[step.src_slot].uuid_path,
                        'well': step.src_well,
                    }
                }, ]
                step_pd.loc[len(step_pd)] = [len(step_pd), i, 'pickUpTip', step.src_slot, step.src_well, '', na, na, na,
                                             na, na]
                ...
                if self._need_delay('aspirate', step.src_sid):
                    command.append({
                        'command': 'delay',
                        'params': {
                            'wait': self.configs.aspirate_delay,
                        }
                    })
                ...
                command.append(
                {
                        ...
                        'params': {
                            'pipette': pipette_id,
                            'volume': self.configs.air_gap_vol,
                            'labware': designer.labware_map[step.src_slot].uuid_path,
                            'well': step.src_well,
                            'offsetFromBottomMm': self.configs.air_gap_offset,
                            'flowRate': aspirate_rate,
                        }
                    })
                if self._need_delay('airgap', step.src_sid):
                    command.append({
                        'command': 'delay',
                        'params': {
                            'wait': self.configs.airgap_delay,
                        }
                    })
                ...
                command.append({
                        ...
                        'params': {
                            'pipette': pipette_id,
                            'labware': designer.labware_map[step.dst_slot].uuid_path,
                            "volume": step.vol + self.configs.air_gap_vol,
                            'well': step.dst_well,
                            'offsetFromBottomMm': d_offset,
                ...
                if self._need_delay('dispense', step.src_sid):
                    command.append({
                        'command': 'delay',
                        'params': {
                            'wait': self.configs.dispense_delay,
                        }
                    })
                ...
                if self._need_delay('blowout', step.src_sid):
                    command.append({
                        'command': 'delay',
                        'params': {
                            'wait': self.configs.blowout_delay,
                        }
                    })
                step_pd.loc[len(step_pd)] = [len(step_pd), i, 'aspirate', step.src_slot, step.src_well,
                                             step.src_sid,
                                             step.vol,
                                             na,
                                             aspirate_rate, a_offset, na]
                if self._need_delay('aspirate', step.src_sid):
                    step_pd.loc[len(step_pd)] = [len(step_pd), i, 'delay', na, na, na, na, na, na, na, self.configs.aspirate_delay]
                    
                step_pd.loc[len(step_pd)] = [len(step_pd), i, 'aspirate', step.src_slot, step.src_well, 'AIR-0000',
                                             self.configs.air_gap_vol,
                                             na,
                                             aspirate_rate, self.configs.air_gap_offset, na]
                
                if self._need_delay('airgap', step.src_sid):
                    step_pd.loc[len(step_pd)] = [len(step_pd), i, 'delay', na, na, na, na, na, na, na, self.configs.airgap_delay]
                    
                    
                step_pd.loc[len(step_pd)] = [len(step_pd), i, 'dispense', step.dst_slot, step.dst_well,
                                             step.dst_sid,
                                             step.vol + self.configs.air_gap_vol,
                                             step.dst_tot_vol,
                                             dispense_rate, d_offset, na]
                if self._need_delay('dispense', step.src_sid):
                    step_pd.loc[len(step_pd)] = [len(step_pd), i, 'delay', na, na, na, na, na, na, na, self.configs.dispense_delay]
                    
                    
                step_pd.loc[len(step_pd)] = [len(step_pd), i, 'blowout', step.dst_slot, step.dst_well, step.dst_sid,
                                             na,
                                             na,
                                             dispense_rate, d_offset, na]
                if self._need_delay('blowout', step.src_sid):
                    step_pd.loc[len(step_pd)] = [len(step_pd), i, 'delay', na, na, na, na, na, na, na, self.configs.blowout_delay]
        ...
        # Validate steps
        try:
            assert step_pd[step_pd['type'] == 'aspirate']['vol'].max() <= self.configs.pipette_max_vol
            assert step_pd[step_pd['type'] == 'aspirate']['vol'].min() >= self.configs.pipette_min_vol
...
#                     print(row.sample_sid, self.configs.n_sources_per_sample)
                ...
                for _, row in well_df.iterrows():
                    if row.part_i > 1: continue
                    d = dict(sid=row.sample_sid, owner=owner, slot=row.ot2_slot, ot2_proc_sid=proc_sid,
                             additional_flags=row.additional_flags)
                    non_zero_i = 0
#                     print(row.sample_sid, self.configs.n_sources_per_sample)
                    for i in range(1, self.configs.n_sources_per_sample + 1):
                        if getattr(row, f'src{i}_vol', 0) > 0:
        ...
        for group_name, group_iter in itertools.groupby(sorted(job_list, key=sort_key), group_key):
            ...
            for i_group, (label_type, label_style, kwargs) in enumerate(grouped_jobs):
                if axes is None:
                    ...
                    lm = LabelMaker(tmpl=self.configs.label_template)
                    axes = lm.create_axes(fig)
