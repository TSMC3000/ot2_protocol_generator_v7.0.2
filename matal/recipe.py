...
def generate_random_compositions(materials, n_comps=30, n_sources_per_comp=None,
    ...
    if feas_mask_func is None:
        if feas_func is None:
            if feas_model is not None:
                feas_mask_func = lambda c: feas_model.predict_proba(c)[:, 1] >= feas_cutoff
        else:
    ...
    while len(valid_comps) < n_comps:
        if method in ['random', 'dirichlet']:
            ...
            _method = 'dirichlet' if rng.integers(0, 5) else 'random'
        ...
        if _method == 'random':
            ...
            masks = np.array([get_mask() for _ in range(batch_size)])
            comps = np.array([
                (rng.dirichlet(m, size=None) + min_frac) / (1 + min_frac) * m
                for m in masks
            ])
        else:
...
class UniformCompositionGenerator:
    ...
    def _build_comp_cache(self):
        ...
        if comps is None:
            ...
            comps = generate_random_compositions(self.materials, n_comps=self.n_comps * self.cache_batch_factor,
